...
func (r *AgentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) { //nolint:gocyclo
...
func (r *AgentReconciler) ensureHelpers(logger logr.Logger) error { //nolint:unparam
...
func (r *AgentReconciler) handleDeletionWithHelper(ctx context.Context, logger logr.Logger, agent *platformv1alpha1.Agent) (ctrl.Result, error) { //nolint:unparam
...
func (r *AgentReconciler) reconcileModelServer(ctx context.Context, agent *platformv1alpha1.Agent) error { //nolint:gocyclo
	logger := log.FromContext(ctx)

	select {
	case <-ctx.Done():
		return fmt.Errorf("context canceled during model server reconciliation: %w", ctx.Err())
	default:
	}
...
func (r *AgentReconciler) reconcileOrchestrator(ctx context.Context, agent *platformv1alpha1.Agent) error { //nolint:gocyclo
	logger := log.FromContext(ctx)

	select {
	case <-ctx.Done():
		return fmt.Errorf("context canceled during orchestrator reconciliation: %w", ctx.Err())
	default:
	}
	...
	if queryURL != "" {
		deployment.Spec.Template.Spec.Containers[0].Env = append(
			...
			corev1.EnvVar{Name: "QUERY_URL", Value: queryURL},
		)
	}
